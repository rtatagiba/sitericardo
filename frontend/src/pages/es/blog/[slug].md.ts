...
// Markdown alternates for agent consumption — /es/blog/{slug}.md returns the
// raw post (frontmatter + body). Advertised via <link rel="alternate"
// type="text/markdown"> on each post.
export async function getStaticPaths() {
  const posts = await getPublishedPosts('blogEs');
...
export async function GET({ props }: APIContext) {
  const post = props.post as CollectionEntry<'blogEs'>;
  const frontmatter = [
    '---',
